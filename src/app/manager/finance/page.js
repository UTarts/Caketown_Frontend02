...
import {
  ...
  ArrowUpRight, FileText, UserCircle, ShieldAlert, Lock, Send
} from "lucide-react";
...
export default function ManagerFinanceHub() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [uRes, lRes, aRes] = await Promise.all([
      ...
      callApi("get_monthly_attendance", { branch_id: session.branch_id, month: finMonth, year: finYear })
    ]);
  ...
  }, [users, ledgerData, attendanceData, searchQuery, finMonth, finYear]);

  const globalFilteredUsers = useMemo(() => {
    if (!globalSearchQuery) return users;
    const q = globalSearchQuery.toLowerCase();
    return users.filter(u => u.name?.toLowerCase().includes(q) || u.department?.toLowerCase().includes(q));
  ...
  const handleLogTransaction = async (e) => {
    ...
    const res = await callApi(endpoint, {
      ...
      manager_id: session.id, // For requests
      actor_id: session.id, // For standard logging
      type: formType,
  ...
  const canLogAnything = canLogAdv;

  // For manager, we rely entirely on the toggle we build in personnel
  const availableTypes = Object.keys(TYPE_MAP);
      ...
      <div className="w-full box-border bg-white/60 dark:bg-neutral-900/40 backdrop-blur-xl border border-gray-200/60 dark:border-neutral-800/60 rounded-3xl shadow-sm p-4 md:p-5">
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 text-orange-600 dark:text-orange-500 mb-1">
            ...
            <span className="text-[10px] font-black tracking-[0.2em] uppercase">Branch Ledger</span>
          </div>
          ...
          <div className="bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-neutral-800 w-full max-w-4xl max-h-[90vh] rounded-t-3xl md:rounded-3xl shadow-2xl animate-in slide-in-from-bottom-full md:zoom-in-95 duration-200 flex flex-col overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden min-h-0">
              ...
              <div className="w-full md:w-1/2 border-b md:border-b-0 md:border-r border-gray-100 dark:border-neutral-900 p-4 md:p-6 overflow-y-auto bg-white dark:bg-[#0a0a0a]">
                ...
                {/* NEW LOGIC: Requesting Admin Approval */}
                {['pre_advance', 'final_advance', 'shop_advance'].includes(formType) && parseFloat(formAmount || 0) > activeUserModal.availAdv && (
                  <div className="mb-6 p-3 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-900/50 rounded-xl flex items-start gap-2 animate-in fade-in zoom-in-95">
                ...
                {canLogAnything ? (
                  <form onSubmit={handleLogTransaction} className="space-y-4">
                    ...
                    {/* NEW: Dynamic Button Styling Based on Limit */}
                    <button
                      type="submit"
                      disabled={formSubmitting}
                      className={`w-full py-4 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-lg active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2 mt-4 min-h-[52px] ${
                        parseFloat(formAmount || 0) > activeUserModal.availAdv && ['pre_advance', 'final_advance', 'shop_advance'].includes(formType) 
                          ? "bg-indigo-500 hover:bg-indigo-600 shadow-indigo-500/20" 
                          : "bg-gray-900 hover:bg-black dark:bg-white dark:hover:bg-gray-200 dark:text-black shadow-gray-900/20 dark:shadow-white/10"
                      }`}
                    >

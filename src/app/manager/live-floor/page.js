...
import {
  ...
  ChevronLeft, ChevronRight, LogIn, X, 
...
const canViewHistory = session ? canRead(session.feature_permissions, 'view_attendance_history') : false;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!canRead(parsed.feature_permissions, 'view_live_attendance')) {
  ...
  const fetchLiveFloor = useCallback(async () => {
    ...
    setLoading(true);
    const res = await callApi("get_live_attendance", { branch_id: session.branch_id, date: viewDate });
    if (res.status === "success") {
      setLiveData(res.data.all_people || []);
    }
    setLoading(false);
  }, [session?.branch_id, viewDate]);
  ...
  useEffect(() => {
    ...
    const fetchInspectorData = async () => {
      ...
      const res = await callApi("get_live_attendance", { branch_id: session.branch_id, date: inspectorDate });
      if (res.status === "success") {

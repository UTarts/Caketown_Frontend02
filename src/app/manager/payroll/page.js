...
import {
  Banknote, Calendar, ChevronDown, Download, FileText,
  Info, Loader2, RefreshCw, Search, Wallet, CheckCircle2,
...
// ─── MAIN COMPONENT ────────────────────────────────────────────────────────
export default function ManagerPayrollEnginePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!canRead(parsed.feature_permissions, 'view_payroll')) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setLoading(true);
    const payrollRes = await callApi("get_monthly_attendance", { month, year, branch_id: session.branch_id });
    if (payrollRes.status === "success") setRows(payrollRes.data || []);
    setLoading(false);
  }, [month, year, session?.branch_id]);

  useEffect(() => { if (session) fetchAll(); }, [session, fetchAll]);

  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      const text = `${row.name || ""} ${row.department || ""} ${row.role || ""}`.toLowerCase();
  ...
  // ─── PERMISSIONS MATRIX ───
  const canProcess = canWrite(session.feature_permissions, 'process_payroll');
  const canDownload = canRead(session.feature_permissions, 'download_salary_slip');
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] md:h-[calc(100vh-2rem)] gap-4 md:gap-5 animate-in fade-in duration-500 text-gray-900 dark:text-neutral-200 font-sans w-full min-w-0 max-w-full overflow-hidden px-3 md:px-0">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-white/60 dark:bg-neutral-900/40 p-4 md:p-5 rounded-3xl backdrop-blur-xl border border-gray-200/60 dark:border-neutral-800/60 shadow-sm w-full shrink-0 mt-3 md:mt-0">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-500 mb-1">
            ...
            <span className="text-[10px] md:text-xs font-black tracking-[0.2em] uppercase truncate">Branch Payroll Engine</span>
          </div>
          <h1 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white tracking-tight truncate">
            Salary Computation - {session.branch_name}
          </h1>
        </div>
      ...
      {/* ── SEARCH ─────────────────────────────────────────────────────── */}
      <div className="relative shrink-0 w-full md:w-96 min-w-0">
        ...
        <div className="bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-neutral-800 rounded-3xl overflow-hidden shadow-sm flex flex-col flex-1 min-w-0 min-h-0 relative">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center flex-1 text-center">
              ...
              <p className="text-sm font-bold text-gray-500">No payroll records match the current period.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse min-w-[1600px]">
                <thead className="sticky top-0 z-30">
                  <tr className="bg-gray-50/95 dark:bg-[#050505]/95 backdrop-blur-md border-b border-gray-200 dark:border-neutral-800 text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap shadow-sm">
                    <th className="p-4 md:p-5 sticky left-0 bg-gray-50/95 dark:bg-[#050505]/95 backdrop-blur-md z-40 border-r border-gray-200 dark:border-neutral-800 shadow-[4px_0_12px_rgba(0,0,0,0.02)]">Personnel</th>
                    ...
                    {canDownload && <th className="p-4 md:p-5 text-center">Action</th>}
                  </tr>
                </thead>
                  ...
                  {filteredRows.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className={`hover:bg-gray-50/50 dark:hover:bg-neutral-900/30 transition-colors group ${isPaid ? 'opacity-70 grayscale-[0.5]' : ''}`}>
                        ...
                        <td className="p-4 md:p-5 sticky left-0 bg-white dark:bg-[#0a0a0a] group-hover:bg-gray-50/50 dark:group-hover:bg-[#111] z-20 border-r border-gray-100 dark:border-neutral-900 shadow-[4px_0_12px_rgba(0,0,0,0.02)] transition-colors">
                        ...
                        <td className="p-4 md:p-5 text-center">
                          {isPaid ? (
                             <div className="flex flex-col items-center">
                               ...
                               <span className="text-[8px] font-bold text-gray-400 mt-1">{new Date(row.processed_at || row.finalized_at || new Date()).toLocaleDateString()}</span>
                             </div>
                          ) : c.isNegative ? (
                            ...
                            <button 
                              ...
                              disabled={!canProcess || c.salaryToPay <= 0} 
                ...
                <tfoot className="sticky bottom-0 z-30">
                  <tr className="border-t-2 border-gray-200 dark:border-neutral-800 bg-gray-50/95 dark:bg-[#050505]/95 backdrop-blur-md font-black shadow-[0_-4px_12px_rgba(0,0,0,0.02)]">
                    <td className="p-4 md:p-5 sticky left-0 z-40 bg-gray-50/95 dark:bg-[#050505]/95 backdrop-blur-md border-r border-gray-200 dark:border-neutral-800">Grand Total</td>
                    ...
                    <td className="p-4 md:p-5 text-center">—</td>
                    {canDownload && <td className="p-4 md:p-5 text-center">—</td>}
                  </tr>
                </tfoot>
              </table>
            </div>
